...
class GaussianDiffusion(nn.Module):
    ...
    ## 加入了健康和异常区域的权重
    def p_losses_(self, x_start, orig, t, cond=None, noise=None, box=None, scale_patch=1, onlybox=False, mask=None, healthy_weight=1.0, anomal_weight=1.0):
        ...
        if mask is not None:
            # Compute the loss for healthy and anomalous regions separately
            healthy_loss = self.loss_fn(model_out * (1 - mask), target * (1 - mask), reduction='mean') * healthy_weight
            anomal_loss = self.loss_fn(model_out * mask, target * mask, reduction='mean') * anomal_weight

            # Combine the losses
            loss = healthy_loss + anomal_loss
        else:
            # Compute the loss for the entire image
            loss = self.loss_fn(model_out, target, reduction='mean')

        return loss, unnormalize_to_zero_to_one(model_out) if self.objective == 'pred_x0' else unnormalize_to_zero_to_one(x - extract(self.sqrt_one_minus_alphas_cumprod, t, x_start.shape) * model_out)
